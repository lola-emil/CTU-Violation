...
if (isset($_POST["reset-password"]) && !isset($_GET["code"])) {
    ...
    if (!password_verify($old_password, $stored_password)) {
        $_SESSION['error'] = "Incorrect ang imong old nga password." . $stored_password;
        header("Location: /CTU-Violation/reset_admin_password.php"); // Redirect back to form
        exit;
    }
    ...
    if ($new_password !== $confirm_password) {
        ...
        $updateQuery = "UPDATE users SET password = '$ang_password' WHERE username = 'admin'";
...
if (isset($_POST["reset-password"]) && isset($_GET["code"])) {
    ...
    if (strlen($new_password) < 8) {
        $_SESSION['error'] = "New password must be at least 8 characters long.";
        header("Location: /CTU-Violation/reset_admin_password.php?code=" . $_GET["code"]);
        exit;
    }
    if (!preg_match('/[A-Z]/', $new_password)) {
        $_SESSION['error'] = "New password must contain at least one uppercase letter.";
...
<body>
    ...
    <br>
    <form
        action="/CTU-Violation/reset_admin_password.php?<?php echo isset($_GET['code']) ? 'code=' . $_GET['code'] : '' ?>"
        method="POST">
        ...
        <a href="/CTU-Violation/forgot_password.php?user=admin">Forgot Password?</a>
    </form>
</body>
